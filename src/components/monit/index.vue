								...
								<el-col :span="6" v-for="(item, index) in equipmentOperationalCondition" :key="index">
									<el-card shadow="always" class="cardItem">
										<div slot="header" class="clearfix">
											...
											<router-link :class="'g-link f14'" style="float: right; " :to="{path:'/Monit/'+item.id}">
										...
										<div class="text item">
											<p
												:style="'border-color:'+(item.runState===0?'green':item.runState===1?'red':item.runState===2?'yellow':item.runState===3?'darkgray':'#CEC9C9')"
											>编号：{{item.deviceNo}}</p>
											<!-- <div class="separate"></div> -->
											<ul>
												<!-- <li>
													<i class="iconfont">&#xe68b;</i>
													{{itemName(item.organizeCode,0)}}：
													<span>{{item.collectData1}}</span>
												</li>
												<li>
													<i class="iconfont">&#xe68b;</i>
													{{itemName(item.organizeCode,1)}}：
													<span>{{item.collectData2}}</span>
												</li>-->
												<!-- <li>
													<i class="iconfont">&#xe68b;</i>
													{{itemName(item.organizeCode,2)}}：
