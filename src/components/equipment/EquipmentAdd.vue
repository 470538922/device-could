<template>
  <div class="equipmentAdd">
    <div class="addCase">
      <div class="top">
        <el-row>
          ...
          <el-button size="small" type="primary" @click="addwarning">
    ...
    <el-dialog title="人员添加" :visible.sync="dialogVisible" width="80%">
      <addPerson :personAddHandler="personAddHandler" :selectShow="true"></addPerson>
    </el-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleChange(value) {
      let name = this.$refs["getName"].getCheckedNodes()[0].pathLabels;
    ...
    handleChange2(value) {
      let name = this.$refs["getName2"].getCheckedNodes()[0].pathLabels;
      name = name[name.length - 1];
      let id = value[value.length - 1];
    ...
    addpersondata() {
      ...
      if (
        this.devicePersonnelInfoBase.find(item => item.workerType === "0") ==
        null
      ) {
        this.devicePersonnelInfoBase.push(
          {
            workerTypeName: "负责",
            workerType: "0",
            content: [{ workerName: user.name, id: user.employeeId }]
          },
          {
            workerTypeName: "维修",
            workerType: "1",
    ...
    addwarning() {
      ...
      if (
        this.sizeForm.deviceClassify === "" ||
        this.sizeForm.organizeCode === "" ||
        this.sizeForm.deviceCategory === ""
      ) {
        subok = false;
      }
      //如果提交通过, 则弹出提示框
