...
<template>
  <div class="redact">
    <div class="add-case">
      <div class="top">
        <el-row>
          <el-button size="small" @click="tback" type="primary" icon="el-icon-arrow-left">返回</el-button>
          <el-button size="small" type="primary" @click="updatewarning">
      ...
      <div class="bottom-case">
        <div class="center">
          <h5>基础信息</h5>
          <el-form ref="baseform" label-width="80px" size="small">
            ...
            <el-form-item label="所属部门">
              ...
              <el-button size="mini" type="primary" @click="dialogVisible1=true">点击修改</el-button>
              <el-dialog title="修改部门" :visible.sync="dialogVisible1" width="300px">
                <div style="overflow: hidden;">
                  ...
                  <div style="text-align:right;padding:10px 20px;">
                    ...
                    <el-button type="primary" @click="orgsave">确 定</el-button>
                  </div>
                </div>
              </el-dialog>
            </el-form-item>
            <el-form :inline="true" style size="small" label-width="80px">
              ...
              <el-form-item label="设备类别">
                ...
                <el-button size="mini" type="primary" @click="dialogVisible3=true">点击修改</el-button>
                <el-dialog title="修改类别" :visible.sync="dialogVisible3" width="300px">
                  <el-col :span="24" style="text-align:center;">
                    <el-cascader
                      placeholder="请选择"
                      :options="ctgoptions"
                  ...
                  <div style="text-align:right;padding:10px 20px;">
                    ...
                    <el-button type="primary" @click="ctgsave">确 定</el-button>
                  </div>
                </el-dialog>
              </el-form-item>
            </el-form>
            ...
            <el-form-item label="负责人员：">
              <span v-if="devicePersonnelInfoBase.find(item=>item.workerType==='0')">
                <el-tag
                  :style="{ margin: '0 5px' }"
                  :key="person.id"
                  v-for="person in devicePersonnelInfoBase.find(item=>item.workerType==='0').content"
                >{{person.workerName}}</el-tag>
              </span>
              <span v-else>（空）</span>
            </el-form-item>
            <el-form-item label="维修人员：">
              <span v-if="devicePersonnelInfoBase.find(item=>item.workerType==='1')">
                <el-tag
                  :style="{ margin: '0 5px' }"
                  :key="person.id"
                  v-for="person in devicePersonnelInfoBase.find(item=>item.workerType==='1').content"
                >{{person.workerName}}</el-tag>
              </span>
              <span v-else>（空）</span>
            </el-form-item>
            ...
            <el-form-item label="检修人员：">
              <span v-if="devicePersonnelInfoBase.find(item=>item.workerType==='2')">
                <el-tag
                  :style="{ margin: '0 5px' }"
                  :key="person.id"
                  v-for="person in devicePersonnelInfoBase.find(item=>item.workerType==='2').content"
                >{{person.workerName}}</el-tag>
              </span>
              <span v-else>（空）</span>
            </el-form-item>
            <el-form-item label="保养人员：">
              <span v-if="devicePersonnelInfoBase.find(item=>item.workerType==='3')">
                <el-tag
                  :style="{ margin: '0 5px' }"
                  :key="person.id"
                  v-for="person in devicePersonnelInfoBase.find(item=>item.workerType==='3').content"
                >{{person.workerName}}</el-tag>
              </span>
              <span v-else>（空）</span>
            </el-form-item>
            ...
            <el-form-item label="操作人员：">
              <span v-if="devicePersonnelInfoBase.find(item=>item.workerType==='4')">
                <el-tag
                  :style="{ margin: '0 5px' }"
                  :key="person.id"
                  v-for="person in devicePersonnelInfoBase.find(item=>item.workerType==='4').content"
                >{{person.workerName}}</el-tag>
              </span>
              <span v-else>（空）</span>
            </el-form-item>
    ...
    <el-dialog title="人员添加" :visible.sync="dialogVisible" width="80%">
  ...
  methods: {
    ...
    update() {
      ...
      this.devicePersonnelInfoBase.forEach(items => {
        _devicePersonnelInfo = _devicePersonnelInfo.concat(
      ...
      let data = {
        ...
        location: this.sizeForm.location.join(","),
    ...
    detail(id) {
      ...
      this.Axios(
        ...
        result => {
          ...
          this.sizeForm.location = this.sizeForm.location.split(",");
          this.sizeForm.location = this.sizeForm.location.map(Number);
    ...
    organdcls() {
      this.Axios(
        ...
        ([res1, res2]) => {
          this.orgoptions = this.filterArray(
            res1.data.data,
      ...
      this.Axios(
        ...
        result => {
          if (result.data.code === 200) {
            ...
            this.location = this.filterArray3(result.data.data, arr);
          }
        },
        ({ type, info }) => {}
      );
    },
    filterArray3(data, parent) {
      let vm = this;
      var tree = [];
      var temp;
      for (var i = 0; i < data.length; i++) {
        if (data[i].parentCode == parent) {
          var obj = data[i];
          temp = this.filterArray3(data, data[i].code);
          if (temp.length > 0) {
            obj.children = temp;
          }
          tree.push(obj);
