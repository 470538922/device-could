      ...
      <div v-show="nextshow" style="margin-top:20px">
        ...
        >
          ...
          <el-form-item label="验证码：" prop="validate">
            <el-input placeholder="短信验证码" v-model="manager.validate" size="small" style="width:40%"></el-input>
            <el-button
              type="primary"
              style="width:38%;"
              size="small"
        ...
        <el-dialog title="用户注册协议" :visible.sync="showDeal" width="600" center append-to-body>
          <div
            style="height:600px;width:100%;overflow:scroll;font-size:12px;line-height:20px;"
            id="deal-content"
            v-html="deal.content"
          ></div>
    ...
    dealMsg() {
      this.Axios(
        ...
        result => {
          ...
          this.deal =
            result.data.data != [] ? result.data.data[0] : { content: "" };
        },
        ({ type, info }) => {}
      );
    },
